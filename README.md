...
2. Run the following command:
      ...
      ```
      ```bash
      CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.run --nproc_per_node=2 main.py --yaml_file configs/hoi_hico_text.yaml --DATA_ROOT "/media/store/lxc/datasets" --MODEL_ROOT "/media/store/lxc/models" --ckpt /media/store/lxc/models/gligen-generation-text-box/diffusion_pytorch_model.bin --name test --batch_size=2 --gradient_accumulation_step 2 --total_iters 500000 --amp true --disable_inference_in_training true       --official_ckpt_name sd1.4/sd-v1-4.ckpt
      ```
      ```bash
