...
import inference
from grounding_input.show_boundingbox import print_boundingbox
...
if __name__ == "__main__":
    ...
    for r in res[0:33]:  # [:2]
        meta_list_new.append(dict(
            ckpt="/media/store/lxc/models/interactdiff/interact-diffusion-v1-1.pth",
            prompt=r["prompt"],
    ...
    # "The rain fills the puddle",
    # "The river flows into the sea"
    #         ],
    #     subject_phrases=["cat","wind","sun","rain","river"],
    #     object_phrases=["mouse","leaves","mountains","puddle","sea"],
    #     action_phrases=["chases","blows","sets","fills","flows"],
    # )) 
    # for index,item in enumerate(meta_list_new):
    #     item["prompt"] = not_hoi_data["prompt"][index]
    #     item["subject_phrases"] = not_hoi_data["subject_phrases"][index]
    #     item["object_phrases"] = not_hoi_data["object_phrases"][index]
    #     item["action_phrases"] = not_hoi_data["action_phrases"][index]
    #     print(item)

    # print(meta_list_new[0])
    # for item in meta_list_new[0:33]:
    #     print_boundingbox(item)
...
'''
10: 存在语义的错误：并不是"chair"
15: 存在数量错误："plant"数量不对
20: 位置关系错误
'''
